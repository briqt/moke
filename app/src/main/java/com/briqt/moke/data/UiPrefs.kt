 ...
 * ↑/↓，翻的是命令历史而不是看输出。
 *
 * - [SMART]  智能（默认）：远端开了鼠标跟踪→发滚轮；不在备用屏（含 mosh 会话——mosh 自己的
 *            备用屏切换已被忽略）→ 滚本地 scrollback；确实在备用屏时按远端是否开启**括号粘贴
 *            模式**区分——开了的是行编辑型程序（Ink/readline 系都会开），不再发方向键；没开的
 *            是翻页器（less/man），保持方向键滚动的既有手感。
 * - [WHEEL]  始终发滚轮：远端支持鼠标但没在当前界面开启跟踪时有用。
 * - [ARROWS] 始终发方向键：v0.1.16 及更早的行为（优先于以上一切判定）。
 */
